...
def extract_subimages(image):
    ...
    #DEBUG
    assert all([crop.shape == (225,225,3) for crop in crops])
    # plt.imshow(image)
    # plt.show()
...
def format_data(data):
    # values = [(value["subimages"],value["labels"],value["bounding_boxes"])
...
def process_data():
    ...
    #pickle.dump(data, open("data.p", "wb"),protocol=4)
    ...
    #pickle.dump(formatted_data, open("data.p", "wb"),protocol=4)
